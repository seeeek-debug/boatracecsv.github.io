...
NOTIFIED_FILE = "data/notified_races.json"
...
def load_notified_history(date_str):
...
def save_notified_history(date_str, notified_list):
    os.makedirs(os.path.dirname(NOTIFIED_FILE), exist_ok=True)
    try:
        with open(NOTIFIED_FILE, "w", encoding="utf-8") as f:
            json.dump({"date": date_str, "races": notified_list}, f, ensure_ascii=False)
...
def run_daily_prediction():
    ...
    already_notified = load_notified_history(date_str)
    new_recommendations = []
    newly_notified_codes = list(already_notified)
    
    for race_code, boat_data_list in races_dict.items():
        # すでに通知済みのレースはスキップ！
        if race_code in already_notified:
            continue
        ...
        if allocated_bets:
            stadium_name = STADIUM_ID_TO_NAME.get(stadium_id, "不明")
            new_recommendations.append({
                'race_code': race_code,
                'stadium_name': stadium_name,
                'bets': allocated_bets
            })
            newly_notified_codes.append(race_code)

    if not new_recommendations:
        return

    # 新しく見つかったレースの買い目だけを送信
    msg_lines = [f"**【推奨買い目・直前情報反映】** ({date_str})\n```text"]
    for rec in new_recommendations:
        msg_lines.append(f"📍 {rec['stadium_name']} (Rコード: {rec['race_code']})")
        for combo, amount, odds in rec['bets']:
            msg_lines.append(f"  [3連単] {combo}  投資: {amount}円  (オッズ: {odds}倍)")
    msg_lines.append("```")
    ...
    # 通知済みに追加して保存
    save_notified_history(date_str, newly_notified_codes)
