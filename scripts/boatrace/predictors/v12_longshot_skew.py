...
def main():
    ...
    prev_logic_venues = ["浜名湖", "芦屋", "尼崎", "下関"]
    current_logic_venues = ["戸田", "江戸川", "蒲郡", "津", "三国", "びわこ"]
    allowed_venues = prev_logic_venues + current_logic_venues
        ...
        for od3_csv in od3_root.glob("**/*.csv"):
            try:
                ...
                for _, row in df_od3.iterrows():
                    ...
                    boat_powers = {}
                    if venue in prev_logic_venues:
                        # 【前のロジック（高回収率のフル版：展示データ・波高補正・コース勝率補正あり）】
