#!/usr/bin/env python3
"""
Fetch real-time tkz (exhibition & weight) preview data for today's races
from race.boatcast.jp TSV files and save to data/previews/tkz/YYYY/MM/DD.csv
"""
...
def parse_tkz_tsv(body_str: str) -> tuple[str, dict]:
    ...
    status_field = lines[1].split("\t")[0].strip() if lines[1] else "1"
    ...
    for boat_num, raw in enumerate(boat_rows, start=1):
        ...
        if len(cols) > 3 and cols[3].strip():
            try:
                w_adj = float(int(cols[3].strip()) / 10.0)
            except ValueError:
                w_adj = ""
        ...
        if len(cols) > 6 and cols[6].strip():
            try:
                t_str = cols[6].strip().replace("+", "")
                tilt_val = float(t_str)
            except ValueError:
                tilt_val = ""
...
def fetch_tkz_for_race(args) -> dict | None:
    stadium_code, race_number, date_str, now_jst = args
    date_formatted = date_str.replace("-", "")
    
    url = f"https://race.boatcast.jp/hp_txt/{stadium_code:02d}/bc_j_tkz_{date_formatted}_{stadium_code:02d}_{race_number:02d}.txt"
    headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)"}
    ...
    try:
        ...
        status_field, boats = parse_tkz_tsv(resp.text)
        if not boats:
            return None
        ...
        for i in range(1, 7):
            b = boats.get(i, {})
            row[f"艇{i}_体重(kg)"] = b.get("weight", "")
            row[f"艇{i}_体重調整(kg)"] = b.get("weight_adj", "")
            row[f"艇{i}_展示タイム"] = b.get("exhibition_time", "")
            row[f"艇{i}_チルト"] = b.get("tilt", "")
...
def main():
    ...
    print(f"=== [START] TKZ Fetcher: {today_str} (JST) ===", flush=True)

    output_dir = project_root / f"data/previews/tkz/{year}/{month}"
    ...
    with ThreadPoolExecutor(max_workers=15) as executor:
        futures = [executor.submit(fetch_tkz_for_race, task) for task in tasks]
