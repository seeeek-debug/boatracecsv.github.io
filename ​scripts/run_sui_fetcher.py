...
and update data/previews/sui/YYYY/MM/DD.csv.
"""
...
# 16方位/文字列 -> 1~8 (1:北, 2:北東, 3:東, 4:南東, 5:南, 6:南西, 7:西, 8:北西)
WIND_DIR_MAP = {
...
def parse_wind_dir(val_str: str) -> int:
    """風向テキストまたはクラス番号から 1..8 に変換"""
    if not val_str:
        return 1
    # 数値(1~16)が渡された場合
...
def fetch_sui_for_race(stadium_code: int, race_number: int, date_str: str) -> dict | None:
    """ボートレース公式サイトの直前情報ページから水面気象データを取得"""
    ...
    try:
        ...
        wind_dir_str = " ".join(wind_dir_class)
        wind_code = parse_wind_dir(wind_dir_str)
        ...
        return {
            "レースコード": race_code,
            "レース場": str(stadium_code),
            "レース回": f"{race_number}R",
            "風速(m)": wind_ms,
            "風向": wind_code,
            "波の高さ(cm)": wave_cm,
            "天候": weather_code,
...
def main():
    ...
    for stadium_code in range(1, 25):
        ...
        for race_number in range(1, 13):
            data = fetch_sui_for_race(stadium_code, race_number, today_str)
            if data:
    ...
    # 既存データと結合して「レースコード」で重複排除（新しいデータ優先）
